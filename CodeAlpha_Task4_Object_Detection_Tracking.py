...
def main():
    ...
    # Video Setup
    cap = cv2.VideoCapture('CodeAlpha_Task4_Video.mp4')
    if not cap.isOpened():
        raise FileNotFoundError("CodeAlpha_Task4_Video.mp4 not found")

    w, h, fps = int(cap.get(3)), int(cap.get(4)), cap.get(cv2.CAP_PROP_FPS)
    fps = fps if fps > 0 else 30.0
    out = cv2.VideoWriter('CodeAlpha_Task4_Output_Tracking.mp4', cv2.VideoWriter_fourcc(*'mp4v'), fps, (w, h))
    ...
    print("✅ Done: CodeAlpha_Task4_Output_Tracking.mp4 saved")
